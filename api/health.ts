...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const healthData = {
    ...
    endpoints: [
      ...
      { path: '/api/admin/auth/login', method: 'POST', description: 'Admin & operator JWT authentication (3 roles)' },
      { path: '/api/admin/rates/update', method: 'POST', description: 'Live bullion & manual rates update' },
      { path: '/api/admin/inventory', method: 'GET,POST', description: 'Inventory CRUD with multi-image support' },
      { path: '/api/admin/orders', method: 'GET,PATCH', description: 'Order reservations desk and status updates' },
      { path: '/api/admin/reviews/moderate', method: 'GET,POST', description: 'Moderate customer reviews' },
